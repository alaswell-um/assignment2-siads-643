...
def filter_by_year_and_metric(l1_grouping, 
    ...
    """
        Filter out the most recent year for analsyis by target metric.
    """
    l1_grouping = l1_grouping[l1_grouping['year'] == year]
    l1_grouping = l1_grouping[level_names + [metric]]
    l2_grouping = l2_grouping[l2_grouping['year'] == year]
...
def create_labels_for_treemap(l1_grouping, 
                              ...
                              metric):
    """
        Creates treemap lables as "occupational_name | percentage_of_all_occupations" and values as percentage_of_all_occupations.
    """ 
    l1 = level_names[0]
    l2 = level_names[1]
    total = l2_grouping[metric].sum() # total seems low, validate later on
...
if __name__ == '__main__':
    ...
    l1_grouping_filtered, l2_grouping_filtered = filter_by_year_and_metric(tgt_l1_grouping, 
                                                                 tgt_l2_grouping,
                                                                 target_levels,
                                                                 year=2015,
                                                                 metric=target_metric)

    l1_grouping_short, l2_grouping_short = apply_short_names(l1_grouping_filtered, 
                                                             l2_grouping_filtered, 
                                                             target_levels)
